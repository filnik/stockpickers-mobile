...
class FakeScannerDao : ScannerDao {
    ...
    override fun observeMomentumLeaders(sort: String, geo: String, limit: Int): Flow<List<TickerEntity>> = rows.map {
        it.take(limit)
    }

    override fun observeGeoCounts(sort: String): Flow<GeoCountsRow> = rows.map { list ->
        GeoCountsRow(total = list.size, usa = list.size, ita = 0, asia = 0)
    }

    override fun observeTicker(ticker: String): Flow<TickerEntity?> = rows.map { list ->
        list.firstOrNull {
            it.ticker ==
                ticker
        }
    ...
    // `replaceAll` is a default method on the DAO, so implementing this is enough for
    // the fake to reproduce the real prune-to-the-published-universe behaviour.
    override suspend fun deleteTickersNotIn(keep: List<String>) {
        rows.value = rows.value.filter { it.ticker in keep }
    }
    ...
    override fun observePriceSeries(ticker: String, rangeKey: String): Flow<PriceSeriesEntity?> = priceSeries.map {
        it[
            ticker to
                rangeKey,
        ]
    ...
    override suspend fun getPriceSeriesFetchedAt(ticker: String, rangeKey: String): Long? = priceSeries.value[
        ticker to
            rangeKey,
    ]?.fetchedAt

    override fun observeProfile(ticker: String): Flow<TickerProfileEntity?> = profiles.map { it[ticker] }

    override suspend fun upsertProfile(profile: TickerProfileEntity) {
        profiles.value = profiles.value + (profile.ticker to profile)
    }

    override suspend fun getProfileFetchedAt(ticker: String): Long? = profiles.value[ticker]?.fetchedAt
}
